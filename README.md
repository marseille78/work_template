...
2. Шаблон страницы открывается из файла _`page-[name].php`_, где `name` передается как значение `$_GET['page]`. При помощи функции `render(tpl_name)` подключаются нужные файлы шаблона блока `/templates/tpl_name.php`.
...
3. Шаблон блока _`/templates/tpl_name.php`_. В нем выводится содержимое выводимого шаблона блока при помощи функции `variable()`, которая в качестве аргумента принимает массив данных **`$var`**, которые передаются для данной страницы.
...
В поле `content`, полученного при помощи функции `render()`, массива `$var` содержится контент непосредственно самого блока. Если этот блок является родительским для более мелких, то они подключаются при помощи функции `render(tpl_name[, data-content[, count-iteration]])`
...
```php
// Результат выполнения следующих строк будет одинаковый
<?=render("tpl_name"); ?>
