...
Стили под каждое разрешение экрана оборачиваются в соответствующие миксины, которые затем подключаются в нужных местах базового файла **`./css/css.scss`**
...
* `_font([name-props], [value-props])` - Для получения значения указанного шрифтового свойства из указанного набора
    * _[name-props] = w_ - получаем значение `[value-props]` толщины шрифта из набора `$g_FontWeight`
    * _[name-props] = s_ - получаем значение `[value-props]` размера шрифта из набора `$g_FontSize`
    * _[name-props] = lh_ - получаем значение `[value-props]` высоты кегля из набора `$g_LineHeight`
    * _[name-props] = f_ - получаем значение `[value-props]` названия импользуемого шрифта из набора `$g_FontFamely`
...
Если *имя-раздела|имя-категории* и *префикс имени-класса* совпадают, допускается имя категории опустить
...
// Рекомендуемые к использованию именования классов: .text-[name-class], .label-[name-class], .message-[name-class], .link-[name-class]
...
Например: _`//** [Раздел/Категория]_[example2-class-name]_xs */`_
