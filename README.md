...
* [CSS](https://github.com/marseille78/work_template#css)
...
* [Шаблоны](https://github.com/marseille78/work_template#Шаблоны)
...
Все нетипичные заготовленные блоки должны храниться в своем файле (**_`/core/mixins/_[cat-name]-[mixin-name].scss`_**) и подключаються в заглавный файл для миксинов (**_`/core/_project_mixins.scss`_**) и первое слово миксина должно содержать **название роздела** в котором он находиться.
...
1. Основной шаблон _`index.php`_, где при помощи функции `render()` подключается шаблон нужной страницы _`page-[параметр GET-запроса].php`_
...
2. Шаблон страницы открывается из файла _`page-[name].php`_, где `name` передается как значение `$_GET['page]`. При помощи функции `render([sass-category]_[main-class-name])` подключаются нужные файлы шаблона блока `/templates/[sass-category]_[main-class-name].php`
```php
<!-- page-example START -->
<h1>Example page</h1>

<?=render([sass-category]_[main-class-name]); ?>

<!-- page-example END -->
```

3. Шаблон блока _`/templates/[sass-category]_[main-class-name].php`_. В нем выводится содержимое выводимого шаблона блока при помощи функции `variable()`, которая в качестве аргумента принимает массив данных **`$var`**, которые передаются для данной страницы. В его поле `$var['content']` - содержится контент непосредственно самого блока. Если этот блок является родительским для более мелких, то они подключаются при помощи функции `render(name-block[, data-content[, count-iteration]])`
   
   * `name-block` - Название полдключаемого блока, из которого берутся данные по-умолчанию, если поле `data` отсутствует, единственный обязательный аргумент
   * `data-content` - Данные о содержимом текущего блока, которые беруться из файла `config.php`
   * `count-iteration` - Принудительное количество повторений отображения текущего блока, для его визуального восприятия. После окончания работы его следует удалить
...
* Если передано только одно (обязательное) значение, то выводятся данные, из одноименного с блоком поля в массиве `$var`, который нам возвращает функция `getVars()`
...
Для получения различных типов конструкций центральной части вызывается функция `render()` в следующем виде:
```php
 <?=render("type_example",getVars('page-preview')); ?>
``` 
, где 2-м аргументом передается функция `getVars()` с ассоциативным массивом, в котором ключами являются части контента сайта, а значениями - списки рендеров шаблонов блоков

Пример отображения содержимого шаблона без вложенности
```php
...
Пример отображения содержимого шаблона с вложенными данными
```php
<!-- sasscat_example START -->
<div>
  <p><?=variable($var['content']); ?></p>
...
Для процесса разработки по-умолчанию включен режим предупреждений (файл `/php/functions.php`):
```php
ini_set('display_errors', 1);
```

По окончании работы его можно выключить:
```php
...
* _`gulp.spritesmith`_ - для создания спрайтов изображений. Использовать в группе с имеющимся миксином (файл _`_icon.scss`_)
...
* Несоответствие функций получения глобальных переменных с документацией
* Стандартные пути также внести в глобальные переменные